...
public class search1Servlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("</head>");
            out.println("<body>");
            int did=Integer.parseInt(request.getParameter("txtdept"));
            String sql="{call sp_getEmployeeByDepartment(?)}";
            try {
                ...
                while(rs.next())
                {
                    Employee e=new Employee(rs.getInt(2),rs.getString(1));
                    lst.add(e);
                }
                for (Employee employee : lst) {
                    out.println(employee.getName()+" "+employee.getSalary());
                }
                request.setAttribute("lstEmp",lst);
